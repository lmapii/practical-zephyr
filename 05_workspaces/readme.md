...
- [Creating a workspace from a repository](#creating-a-workspace-from-a-repository)
...
- [Zephyr with West](#zephyr-with-west)
  ...
  - [Setting up the manifest repository and file](#setting-up-the-manifest-repository-and-file)
  - [Adding a toolchain](#adding-a-toolchain)
  - [Manifest imports](#manifest-imports)
- [Switching boards](#switching-boards)
- [Zephyr as dependency](#zephyr-as-dependency)
...
We'll be using the [development kit for the nRF52840][nordicsemi-nrf52840-dk] and the [STM32 Nucleo-64 development board][stm-nucleo], but you can follow along with any target - real or virtual.
...
We've been using [West][zephyr-west] a lot now to build, debug and flash our project. It comes bundled with the [nRF Connect SDK][nrf-connect-sdk], so until now, we didn't have to install it at all. Without sourcing any paths, e.g., from a `setup.sh` script that we created at the very beginning of this article series, I don't have _West_ installed:
...
Having initialized a _West workspace_, `west config` by default uses this _local_ configuration file to store its configuration options. _West_ also supports storing configuration options globally or even system-wide. Have a look at the  [official documentation][zephyr-west-config] in case you need to know more.

Let's get rid of the configuration option and finally run [`west update`][zephyr-west-basics-init-update] as suggested by the output we got in our call to `west init`:
...
With the `revision` key you can tell _West_ to check out either a specific _branch_, _tag_, or commit hash. Without specifying the `revision`, at the time of writing _West_ defaults to the `master` branch. Since [Doxygen Awesome][doxygen-awesome] uses `main` as a development branch, we have to explicitly specify it.
...
Great! Now _West_ populated our dependencies and we have [Doxygen Awesome][doxygen-awesome] available in the specified `path`:
...
The folder name `deps` is again just a personal choice. I have borrowed this convention from Mike Szczys' talk ["Manifests: Project Sanity in the Ever-Changing Zephyr World"][zephyr-ds22-manifests] from the 2022 Zephyr Developer Summit.
...
In this section, we recreate the modified **Blinky** application that we've seen in the previous article, where we failed to compile the application for the [STM32 Nucleo-64 development board][stm-nucleo] since we relied upon the sources from a globally installed [nRF Connect SDK][nrf-connect-sdk] - which doesn't include [STM32 HAL][zephyr-hal-stm32].
...
Try to run `west build` shows us that we're not just missing the Zephyr source code, but we're also still lacking the Zephyr-specific [extension commands][zephyr-west-cmd-ext], e.g., `build`:
...
As we've seen in previous articles, e.g., `build` and `flash` are Zephyr-specific [extension commands][zephyr-west-cmd-ext] that are not included by a plain _West_ installation.


### Setting up the manifest repository and file

For our workspace structure, we're using what's known as a ["T2 star topology"][zephyr-west-workspace-topologies]: In such a topology, the _manifest repository_ does not only contain the _manifest file(s)_ but also contains all application files.
...
Before running `west update`, let's add Zephyr as a project in `manifest.projects` just like what we've done for the [Doxygen-Awesome][doxygen-awesome] dependency before:
...
Now, we have a complete "T2 star topology" workspace, where the Zephyr dependency is placed in a separate `deps` folder in the _topdir_ (as suggested by Mike Szczys in his talk ["Manifests: Project Sanity in the Ever-Changing Zephyr World"][zephyr-ds22-manifests]):
...
The term ["Zephyr SDK"][zephyr-sdk] refers to the toolchains that must be provided for each of Zephyr’s supported architectures. With our manifest, we only add Zephyr's _sources_ to our workspace. The required tools, however, e.g., the GCC compiler for ARM Cortex-M, are **not** included.
...
- or something like [Nordic's toolchain manager][nrf-connect-mgr],
...
- An installation of the _Zephyr SDK_, containing the architecture specific toolchain. Zephyr's official documentation includes a dedicated section on the [Zephyr SDK][zephyr-sdk] and its installation instructions.
...
In the first article of this series, we installed the correct SDK for the [nRF52840 Development Kit][nordicsemi-nrf52840-dk] using [Nordic's toolchain manager][nrf-connect-mgr]. This installation contains not only the Zephyr SDK but also the host dependencies that I need for building Zephyr applications for nRF devices.

Since both, the STM32 and the nRF52840, are ARM MCUs, the installed Zephyr SDK already also contains the toolchain for building for the [STM32 Nucleo-64 development board][stm-nucleo]. All I need to do is provide the required environment variables.
...
With our workspace set up and our knowledge about Zephyr's imports, we're ready to add support for the [STM32 Nucleo-64 development board][stm-nucleo]. The first thing we need is an overlay file to specify the `/chosen` LED node:
...
The manifest repository is not affected since - as we've seen before - West does not touch the manifest repository at all. This behavior is also explained in the [official documentation][zephyr-west-manifest-rev]. It is important that you do not modify the `manifest-rev` branch and that you don't push it to your remote since West recreates and resets the `manifest-rev` branch on each execution of `west update` command. Any changes would be **lost**.

You need to be especially aware of this behavior if you're using _West_ in a ["T3 forest topology"][zephyr-west-workspace-topologies] or, e.g., if you're using a separate repository for shared code. E.g., we could add the following `shared/dummy` project to our workspace:
...
- Watch Mike Szczys' talk ["Manifests: Project Sanity in the Ever-Changing Zephyr World"][zephyr-ds22-manifests] from the 2022 Zephyr Developer Summit, lots of what you've read here comes from this presentation!
